...
fun AddWatermarkScreen(
    ...
    viewModel: AddWatermarkViewModel = viewModel()
) {
    ...
    var selectedPosition by remember { mutableStateOf(WatermarkPosition.CENTER) }
    ...
    val saveLauncher = rememberLauncherForActivityResult(
        ...
        if (outputUri != null) {
            viewModel.addWatermark(outputUri, watermarkText, opacity, selectedPosition)
        }
    }

    when (val state = uiState) {
        is AddWatermarkUiState.Processing -> ProgressContent(
            progress = state.progress,
            operationName = "Adding Watermark",
            onCancel = { viewModel.cancel() },
            modifier = modifier
        )
        is AddWatermarkUiState.Success -> SuccessContent(
            operationName = "Watermark added successfully",
            outputInfo = "Saved to: ${state.outputUri.lastPathSegment}",
            onNewOperation = { viewModel.reset(); onBack() },
            modifier = modifier
        )
        is AddWatermarkUiState.Error -> ErrorContent(
            ...
            Scaffold(
                topBar = { PdfTopAppBar(title = "Add Watermark", onBack = onBack) },
                bottomBar = {
                    BottomActionBar(
                        primaryText = "Apply Watermark",
                        enabled = inputUri != null && watermarkText.isNotBlank(),
                        onPrimary = { saveLauncher.launch("watermarked.pdf") },
                        onCancel = onBack
                    )
                },
                modifier = modifier
                ...
                Column(
                    ...
                    Text(
                        "Opacity: ${(opacity * 100).roundToInt()}%",
                    ...
                    val positions = listOf(
                        WatermarkPosition.TOP_LEFT to "Top Left",
                        WatermarkPosition.TOP_CENTER to "Top Center",
                        WatermarkPosition.TOP_RIGHT to "Top Right",
                        WatermarkPosition.CENTER to "Center",
                        WatermarkPosition.BOTTOM_LEFT to "Bottom Left",
                        WatermarkPosition.BOTTOM_CENTER to "Bottom Center",
