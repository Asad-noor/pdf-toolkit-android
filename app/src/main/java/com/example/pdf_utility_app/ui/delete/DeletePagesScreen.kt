...
fun DeletePagesScreen(
    ...
    val saveLauncher = rememberLauncherForActivityResult(
        ...
        if (outputUri != null) {
            viewModel.delete(outputUri, parsePageNumbers(pagesInput))
        }
    }

    when (val state = uiState) {
        is DeletePagesUiState.Processing -> ProgressContent(
            progress = state.progress,
            operationName = "Deleting Pages",
            onCancel = { viewModel.cancel() },
            modifier = modifier
        )
        is DeletePagesUiState.Success -> SuccessContent(
            operationName = "Pages deleted successfully",
            outputInfo = "Saved to: ${state.outputUri.lastPathSegment}",
            onNewOperation = { viewModel.reset(); onBack() },
            modifier = modifier
        )
        is DeletePagesUiState.Error -> ErrorContent(
            ...
            Scaffold(
                topBar = { PdfTopAppBar(title = "Delete Pages", onBack = onBack) },
                bottomBar = {
                    BottomActionBar(
                        primaryText = "Delete & Save",
                        enabled = inputUri != null && pagesInput.isNotBlank(),
                        onPrimary = { saveLauncher.launch("modified.pdf") },
                        onCancel = onBack
                    )
                },
                modifier = modifier
                ...
                Column(
                    ...
                    OutlinedTextField(
                        ...
                        label = { Text("Pages to delete (e.g. 1,3,5-7)") },
                        placeholder = { Text("2,4,6-8") },
